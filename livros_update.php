    ...
    <form action="livros_update.php" method="post">
        <input type="hidden" name="id" value="<?= $livro['id'] ?>" />
            <div class="form-group">
                ...
                <input class="form-control" type="text" required name="titulo" value="<?= $livro['titulo'] ?>" />
            </div>
                ...
                <select name="genero" class="form-select">
                    <?php foreach($generos as $g): ?>
                        <option <?= ($g['id'] == $livro['id_genero']) ? 'selected' : '' ?> value="<?= $g['id'] ?>"><?= $g['nome'] ?></option>
                            <!-- Se $g[id] for igual a $livro['id_genero'] vai colocar 'selected' no option, se não, não é colocado-->
                    <?php endforeach ?>
                </select>
